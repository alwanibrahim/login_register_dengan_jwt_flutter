...
class ApiAuth {
  ...
  Future<LoginResponse?> getToken(String email, String password) async {
    try {
      ...
      print("Response Data: ${response.data}"); // Debugging: Log the response data

      if (response.statusCode == 200 && response.data is Map<String, dynamic>) {
        final loginResponse = LoginResponse.fromJson(response.data);
        print("Token received: ${loginResponse.token}"); // Debugging: Log the token received
      ...
      print("DioError di getToken: ${e.response?.data ?? e.message}. Please check your credentials.");
      return null;
    } catch (e) {
